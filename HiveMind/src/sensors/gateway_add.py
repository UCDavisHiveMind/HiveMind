...
sensor_name = 'gateway_add'

update_on_fail = False
enabled = True

class gateway_add(SensorSpec):
#- default enable state on initialization
	def __init__(self, name, enabled=True):
		super(gateway_add, self).__init__(name, enabled)
		print ">>> INITING gateway_add"
        ...
        #- note: not all sensors will have a response
			...
			for x in observed['data']['parsed']:
				if not x in reference['data']['parsed']:
					found[x]=None
			self.message = "New gateway not in baseline found"
